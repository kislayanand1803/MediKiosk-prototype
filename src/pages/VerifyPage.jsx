 ...
 * ==========================================
 * This page acts as a public-facing Digital Health Pass verification system.
 * It is accessed when a doctor or nurse scans the patient's QR code with their phone/tablet.
 * It reads the token from the URL, securely fetches the triage status from Supabase,
 * and displays an authenticated record without exposing the entire medical history.
 */
export default function VerifyPage() {
  // --- URL PARAMETER EXTRACTION ---
  // Grabs the data embedded in the QR code link generated by the Success Page
   ...
   * ==========================================
   * Looks up the token number in the Supabase database.
   * If found, it retrieves the patient's basic triage status (Red Flag vs Routine)
   * and their chief complaint to show the nurse/doctor.
   */
  useEffect(() => {
    async function verifyToken() {
      // If no token is found in the URL, stop loading
      if (!token) {
        setLoading(false);
        return;
      }

      // Query the database for the matching token
  ...
  /**
   * Utility Function:
   * Strips the AI provenance emojis out of the text so the printed pass looks highly formal.
   */
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-8 flex justify-center items-start print:bg-white print:p-0">
      <div className="max-w-lg w-full bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden print:shadow-none print:border-none">
        ...
        <div className="p-6 space-y-6">
          {/* Patient Details (Uses URL fallback data if DB fetch was restricted by security rules) */}
          ...
          {/* Conditional Clinical Data Rendering */}
          {patient ? (
            <>
              {/* Dynamic Alert Box: Red for Emergencies, Blue for Routine */}
              <div
            ...
            // Fallback UI if the token exists but patient details are blocked by Row Level Security
