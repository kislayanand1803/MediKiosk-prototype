...
import {
  ...
  Sun,
  Moon,
  Image as ImageIcon, // MODULE B: Added icon for Document Viewer
} from "lucide-react";
...
// Import the newly built Module B Timeline component
...
export default function DoctorDashboard() {
  ...
  // MODULE B: Document Viewer Modal State
  ...
  // MODULE B: Fetches the heavy text fields and newly extracted OCR arrays
  const handleSelectPatient = async (pat) => {
    ...
    const { data, error } = await supabase
      .from("patients")
      // Explicitly fetching the Module B jsonb columns
      .select(
        ...
        <div className="max-w-md w-full bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="bg-blue-500/10 p-2 rounded-lg border border-blue-500/20 text-[10px] text-blue-300 text-center font-medium">
              Demo Credentials - ID:{" "}
              <span className="font-bold text-blue-200">Hidden</span> | Pass:{" "}
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-gray-100 dark:bg-slate-950 p-3 sm:p-4 md:p-6 transition-colors duration-200 flex flex-col">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 overflow-hidden">
            ...
            {!selectedPatient ? (
              ...
              <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800 p-4 sm:p-6 h-full overflow-y-auto space-y-6 relative transition-colors duration-200">
                ...
                {/* MODULE B: Extracted Document/Lab Data & Timeline */}
                <div className="mt-6">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    {/* MODULE B: Original Viewer Trigger */}
                    {selectedPatient.document_images?.length > 0 && (
                      <button
                      ...
                      >
                        <ImageIcon size={14} /> View Original Scans
                      </button>
                    )}
                  </div>
      ...
      {/* MODULE B: Original Document Viewer Modal */}
      {showDocViewer && selectedPatient && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col h-[90vh]">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 rounded-t-2xl">
              <div>
                <h3 className="text-slate-900 dark:text-white text-base font-black flex items-center gap-2">
                  <ImageIcon className="text-emerald-500" size={20} /> Original
                  Patient Uploads
                </h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-100 dark:bg-slate-900 space-y-6">
              {selectedPatient.document_images?.map((doc, idx) => (
                <div
                  key={idx}
